...
def period_calculator(dataframe):
    """
    Determines the length of year in days. Needed for leap-year handling.
    """
...
def temp(dataframe):
    ...
    """
    year, days = period_calculator(dataframe)
    key1 = 'Min Temp (°C)'
    key2 = 'Max Temp (°C)'
...
def place_that(name):
    ...
    try:
        ...
        with open(location, 'r') as f:
            ...
            for count, row in enumerate(verifier):  # TODO look into removing this sections as Python3 handles BOM
...
def data_unpacker(matches, base, plots):
    """
    ...
    for match in matches:
        ...
        if len(match) > 1:
            ...
            print(station['Station Name'] + ' ID:' + station['Climate Identifier'] + ' for ' + station['Year'][0])

            length = len(period_calculator(station)[1])
            empty = np.ma.masked_array(np.zeros((length,)), mask=np.ones((length,)))

            if True not in (temp(station)[0], precip(station)[0], ddays(station, base)[0]):
                csv_data['Date'].extend(period_calculator(station)[1])
...
def daily_stations(base, plots):
    ...
    csv_list = data_unpacker(matches, base, plots)
...
if __name__ == "__main__":
    ...
    try:
        base = float(input("What base for degree-days? [10.0]"))
    except ValueError:
        pass
    finally:
    ...
    make_plots = False
    plots = input("Do you want to produce plots of data? y/[n]")
    if plots in ('y', 'Y'):
        make_plots = True
