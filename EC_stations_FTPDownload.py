...
def download_hourly(stats,OutDir):
    ...
    stats = stats[stats[time_step +' First Year' ].notnull()] #stats[:][~np.isnan(stats[time_step +' First Year' ])] #get data
    ...
    for index, row in stats.iterrows():
        ...
        if 'QUEBEC' in row['Province']: #\
            ...
            if not np.isnan(row[time_step+ ' First Year']) and not np.isnan(row[time_step+ ' Last Year']):
                ...
                print(row['Name'])
                outrep1 = OutDir + time_step + '\\' + row['Province'].replace(' ','_') + '\\' +  row['Climate ID'] + '_' + row['Name'].replace('\\','_').replace('/','_').replace(' ','_')
                # create if necessary
                ...
                for y in range(int(row[time_step+ ' First Year']),int(row[time_step+ ' Last Year']+1)):
                    for m in range(1,13):
                        URL = 'http://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID=' + str(row['Station ID']) + '&Year=' + str(y) + '&Month=' + str(m) + '&' + timeframe + '&submit= Download+Data'
                        r = requests.get(URL)
                        outfile = outrep1 +'\\'+ row['Climate ID'] +  '_' +  row['Name'].replace('\\','_').replace('/','_').replace(' ','_') + '_' + str(y) + '_' + str(m).zfill(2) + '_' + time_step + '.csv'
...
def download_daily(stats,OutDir):
    ...
    for d in range(0,len(stats['Name'])):
        ...
        if not np.isnan(stats[time_step+ ' First Year'][d]) and not np.isnan(stats[time_step+ ' Last Year'][d]):
            # output directory name
            outrep1 = OutDir + time_step + '\\' + stats['Province'][d].replace(' ','_') + '\\' +  stats['Climate ID'][d] + '_' + stats['Name'][d].replace('\\','_').replace('/','_').replace(' ','_')
            # create if necessary
            if not glob.os.path.exists(outrep1):
                glob.os.makedirs(outrep1)
            # loop through years and download
            for y in range(int(stats[time_step+ ' First Year'][d]),int(stats[time_step+ ' Last Year'][d]+1)):
                ...
                URL = 'http://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID=' + str(stats['Station ID'][d]) + '&Year=' + str(y) + '&' + timeframe + '&submit= Download+Data'
                r = requests.get(URL)
                outfile = outrep1 +'\\'+ stats['Climate ID'][d] +  '_' +  stats['Name'][d].replace('\\','_').replace('/','_').replace(' ','_') +  '_' + str(y) + '_' + time_step + '.csv'
