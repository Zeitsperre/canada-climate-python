...
def period(data_frame):
    ...
    """
    period = np.array(data_frame['Date/Time'], dtype='datetime64[D]')
...
def temp(data_frame):
    ...
    """
    year, days = period(data_frame)
    key1 = 'Min Temp (C)'
    key2 = 'Max Temp (C)'
    min_temp_raw = data_frame[key1]
    max_temp_raw = data_frame[key2]
...
def precip(data_frame):
    ...
    tot_ppt_raw = data_frame[key1]
    snow_raw = data_frame[key2]
...
def ddays(data_frame, base=10):
    ...
    # if data_frame['Climate Identifier'] == '7027329':
    #    stop()
    key = 'Degree Days >' + str(base) + ' Deg C'
    year, days = period(data_frame)

    if temp(data_frame)[0] is False:
        return False, key, year

    min_temp_ma, max_temp_ma = temp(data_frame)[1:3]
...
def data_unpacker(matches, base, make):
    """
...
def daily_stations(base, make):
    ...
    csv_list = data_unpacker(matches, base, make)
...
if __name__ == "__main__":
    ...
    """
    base_deg = raw_input("What base for degree-days?[10]")
    if type(base_deg) not in (int, float):
        base_deg = 10

    print 'base =', base_deg, 'deg C'
    ...
    daily_stations(base_deg, make_plots)
